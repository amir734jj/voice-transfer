...
internal sealed class NAudioLoopbackRecorder : IAudioRecorder
{
    ...
    public NAudioLoopbackRecorder()
    {
        ...
        Log.Information("WASAPI loopback format: {Rate}Hz, {Ch}ch, {Enc}",
            sourceRate, sourceChannels, _capture.WaveFormat.Encoding);
        ...
        _capture.DataAvailable += (_, e) =>
        {
            ...
            if (sourceChannels > 1)
            {
                floats = DownmixToMono(floats, sourceChannels);
            }

            if (sourceRate != Constants.SampleRate)
            {
                floats = Resample(floats, sourceRate, Constants.SampleRate);
            }
